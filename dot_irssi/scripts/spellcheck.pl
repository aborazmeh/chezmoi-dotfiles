...
sub spellcheck_key_pressed
{
    ...
    # we found a mistake, print suggestions in status bar
    $sb_msg = "Spellscheck: %R" . join(", ", @suggestions);
    Irssi::statusbar_items_redraw('spellcheck');
}
sub spellcheck_statusbar {
    my ($sb_item, $get_size_only) = @_;

    $sb_item->default_handler($get_size_only, "$sb_msg", undef, 1);
}
...
Irssi::statusbar_item_register('spellcheck', undef, 'spellcheck_statusbar');
